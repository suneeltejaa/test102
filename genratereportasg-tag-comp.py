...
def getasgclient(region,creddict):
...
def getasgdata(dat,region):
    li = []
    if 'Tags' in dat.keys():
        own,bu,appid,env = (gettagdata(dat['Tags']))
    else:
        own,bu,appid,env = 'No Owner Tag','No BU Tag','No AppID Tag','No Environment Tag'
    li.append(dat['AutoScalingGroupName'])
    li.append(dat['CreatedTime'])
    li.append(dat['VPCZoneIdentifier'])
    li.append(str(dat['AvailabilityZones']))
    li.append(own)
    li.append(bu)
    li.append(appid)
    li.append(env)
...
creds = getcreds()
for keys in creds:
    csvdata = [['AutoScalingGroupName','CreatedTime','VPCZoneIdentifier',
                'AvailabilityZones','Owner','BU','AppID','Environment','Region']]
    if keys != 'DEFAULT' and keys != 'default':
        if keys.split('-')[3] != 'Level1':
            ...
            for region in regions:
                ...
                ll = []
                client = getasgclient(region,creddict)
                out = client.describe_auto_scaling_groups(MaxRecords=100)['AutoScalingGroups']
                if len(out) !=0:
                    for data in out:
                        ll =  getasgdata(data,region)
                        if ll[4] == 'No Owner Tag' or ll[5] == 'No BU Tag' or ll[6] == 'No AppID Tag' or ll[7] == 'No Environment Tag':
                            csvdata.append(ll)
            if keys.split('-')[0] not in acctnames.keys():
                fname = keys.split('-')[0]+'-'+'-asg-report.csv'
            else:
                fname =acctnames[keys.split('-')[0]]+'-asg-report.csv'
            outdir = expanduser("~")+'\Documents\\reports\\asg-tag-comp\\'
